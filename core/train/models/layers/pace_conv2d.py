...
class PACEConv2d_Split2x(nn.Module):
    ...
    def build_parameters(self) -> None:
        ...
        self.weight_1 = nn.Parameter(
            self.scale
            * torch.zeros(
                [2, self.in_channels // 2, self.out_channels // 2, self.n_modes[0]],
                dtype=torch.cfloat,
            )
        )
        self.weight_2 = nn.Parameter(
            self.scale
            * torch.zeros(
                [2, self.in_channels // 2, self.out_channels // 2, self.n_modes[1]],
                dtype=torch.cfloat,
            )
        )
        ...
        if dim == -2:
            ...
            if n_mode == x_ft.size(-2):  # full mode
                ...
                out_ft = self.get_zero_padding(
                    [bs, 2, c // 2, x_ft.size(-2), x_ft.size(-1)], x.device
                )
    ...
    def forward(self, x: Tensor) -> Tensor:
        x = self._ffno_forward(
            x, dim=-1
        )  # do 1d fft and then do group-wise multiplication
        x = self._ffno_forward(
            x, dim=-2
        )  # do 1d fft and then do group-wise multiplication
...
class PACEConv2d_Split4x(nn.Module):
    ...
    def build_parameters(self) -> None:
        ...
        self.weight_1 = nn.Parameter(
            self.scale
            * torch.zeros(
                [4, self.in_channels // 4, self.out_channels // 4, self.n_modes[0]],
                dtype=torch.cfloat,
            )
        )
        self.weight_2 = nn.Parameter(
            self.scale
            * torch.zeros(
                [4, self.in_channels // 4, self.out_channels // 4, self.n_modes[1]],
                dtype=torch.cfloat,
            )
        )
    ...
    def _ffno_forward(self, x, dim=-2):
        ...
        if dim == -2:
            ...
            if n_mode == x_ft.size(-2):  # full mode
                ...
                out_ft = self.get_zero_padding(
                    [bs, 4, c // 4, x_ft.size(-2), x_ft.size(-1)], x.device
                )
                out_ft[..., :n_mode, :] = torch.einsum(
                    "bgixy,giox->bgoxy", x_ft[..., :n_mode, :], self.weight_1
                )
