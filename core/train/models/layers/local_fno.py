    ...
    def forward(self, x, indices=0):
        ...
        if self.output_scaling_factor is not None:
            mode_sizes = tuple(
                [
                    int(round(s * r))
                    for (s, r) in zip(mode_sizes, self.output_scaling_factor)
                ]
            )
...
class DomainPadding(nn.Module):
    ...
    def __init__(
        self, domain_padding, padding_mode="one-sided", output_scale_factor=None
    ):
        super().__init__()
    ...
    def pad(self, x):
        ...
        if self.output_scale_factor is None:
            ...
            self.output_scale_factor = [float(self.output_scale_factor)] * len(
                resolution
            )
        ...
        try:
            ...
            for scale_factor in self.output_scale_factor:
                ...
                output_pad = [
                    int(round(i * j)) for (i, j) in zip(scale_factor, output_pad)
                ]

            if self.padding_mode == "symmetric":
                # Pad both sides
                unpad_indices = (Ellipsis,) + tuple(
                    [slice(p, -p, None) for p in output_pad]
                )
                ...
                # One-side padding
                unpad_indices = (Ellipsis,) + tuple(
                    [slice(None, -p, None) for p in output_pad]
                )
            ...
            for scale_factor in self.output_scale_factor:
                if isinstance(scale_factor, (float, int)):
                    scale_factor = [scale_factor] * len(resolution)
                out_put_shape = [
                    int(round(i * j)) for (i, j) in zip(scale_factor, out_put_shape)
                ]
...
class FNOBlocks(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        if norm is None:
            ...
            self.norm = nn.ModuleList(
                [
                    MyLayerNorm(out_channels, data_format="channels_first")
                    for _ in range(n_layers * self.n_norms)
                ]
            )
        else:
    ...
    def forward(self, x, index=0):
        ...
        if self.convs.output_scaling_factor is not None:
            x_skip_fno = resample(
                x_skip_fno,
                self.convs.output_scaling_factor,
                list(range(-len(self.convs.output_scaling_factor), 0)),
            )
...
def resample(x, res_scale, axis):
    ...
    if len(axis) == 2:
        return F.interpolate(
            x, size=new_size, mode="bicubic", align_corners=True, antialias=True
        )
