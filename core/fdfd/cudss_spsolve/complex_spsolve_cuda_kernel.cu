...
#define CUDSS_CALL_AND_CHECK(call, status, msg)                                \
  do {                                                                         \
    ...
    if (status != CUDSS_STATUS_SUCCESS) {                                      \
      printf("Example FAILED: CUDSS call ended unsuccessfully with status = "  \
             "%d, details: " #msg "\n",                                        \
             status);                                                          \
      CUDSS_EXAMPLE_FREE;                                                      \
      return -2;                                                               \
...
template <typename T>
int cuDSSSpsolveCUDALauncher(int *csr_offsets_d, int *csr_columns_d,
                             T *csr_values_d_raw, T *b_values_d_raw,
                             T *x_values_d_raw, int n, int nrhs, int nnz,
                             int _mtype) {
  // mtype = 0,1,2,3,4
  // CUDSS_MTYPE_GENERAL,
  ...
  // CUDSS_MTYPE_HPD
  using ComplexT =
      typename std::conditional<std::is_same<T, float>::value, cuFloatComplex,
                                cuDoubleComplex>::type;
  constexpr auto CUDA_DTYPE =
      std::is_same<T, float>::value ? CUDA_C_32F : CUDA_C_64F;
  ...
  CUDSS_CALL_AND_CHECK(cudssMatrixCreateCsr(&A, nrows, ncols, nnz,
                                            csr_offsets_d, NULL, csr_columns_d,
                                            csr_values_d, CUDA_R_32I,
                                            CUDA_DTYPE, mtype, mview, base),
  ...
  // CUDA_CALL_AND_CHECK(cudaMemcpy(x_values_h, x_values_d_raw, nrhs * n *
  // sizeof(cuComplex),
  //                     cudaMemcpyDeviceToHost), "cudaMemcpy for x_values");
  // printf("%d %d %d\n", n, nrhs, nnz);
  // printf("%f %f\n", x_values_h[0].x, x_values_h[0].y);

  // CUDA_CALL_AND_CHECK(cudaMemcpy(x_values_h, x_values_d, nrhs * n *
  // sizeof(cuComplex),
  //                     cudaMemcpyDeviceToHost), "cudaMemcpy for x_values");
  // cudaDeviceSynchronize();
  // printf("%f %f\n", x_values_h[0].x, x_values_h[0].y);
...
template int cuDSSSpsolveCUDALauncher(int *csr_offsets_d, int *csr_columns_d,
                                      float *csr_values_d_raw,
                                      float *b_values_d_raw,
                                      float *x_values_d_raw, int n, int nrhs,
                                      int nnz, int _mtype);

template int cuDSSSpsolveCUDALauncher(int *csr_offsets_d, int *csr_columns_d,
                                      double *csr_values_d_raw,
                                      double *b_values_d_raw,
                                      double *x_values_d_raw, int n, int nrhs,
                                      int nnz, int _mtype);
