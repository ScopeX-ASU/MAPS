...
__all__ = ["FourPortCoupler"]


class FourPortCoupler(N_Ports):
    def __init__(
        self,
        ...
        port_cfgs = dict(
            ...
            out_port_2=dict(
                type="box",
                direction="y",
                center=[0.0, (port_len[1] + box_size[1]) / 2],
    ...
    def init_monitors(self, verbose: bool = True):
        rel_width = 2
        if verbose:
            lg.info("Start generating sources and monitors (FourPortNSEW) ...")
        pml = self.sim_cfg["PML"][0]

        port_len_x = self.port_cfgs["in_port_1"]["size"][0]
        port_len_y = self.port_cfgs["out_port_2"]["size"][1]
        offset_x = 0.2 + pml
        offset_y = 0.2 + pml
        ...
        refl_slices = [
            self.build_port_monitor_slice(
                ...
                rel_loc=(offset_x + 0.1) / port_len_x,
                rel_width=rel_width,
            )
        ]
        ...
        out_slices = [
            ...
            self.build_port_monitor_slice(
                port_name="out_port_3",
                slice_name="out_slice_3",
    ...
    def norm_run(self, verbose: bool = True):
        if verbose:
            lg.info("Start normalization run (FourPortNSEW) ...")
        ...
        norm_monitor_profiles = [
            ...
            self.build_norm_sources(
                source_modes=("Ez1",),
                input_port_name="out_port_3",
                input_slice_name="out_slice_3",
                wl_cen=self.sim_cfg["wl_cen"],
                wl_width=self.sim_cfg["wl_width"],
                n_wl=self.sim_cfg["n_wl"],
                # solver=self.sim_cfg["solver"],
