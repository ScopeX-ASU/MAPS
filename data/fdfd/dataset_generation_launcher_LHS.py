...
def metacoupler_launcher(queue):
    ...
    while not queue.empty():
        try:
            rand_seed, gpu_id, port_len, init_weight_idx = (
                queue.get_nowait()
...
if __name__ == "__main__":
    ...
    for i in range(len(port_lens)):
        ...
        lhs_samples = generate_lhs_samples(
            num_per_port_len, dim, lower_bounds, upper_bounds, seed=42
        )
    ...
    for seed in range(taskid_begin, taskid_end):
        ...
        queue.put(
            (seed, seed % num_gpus, port_len, init_weight_idx)
        )  # Convert init_weight to a list for subprocess
