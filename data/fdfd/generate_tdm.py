...
def tdm_opt(device_id, operation_device, each_step=False, include_perturb=False, perturb_probs=[0.05, 0.1, 0.15]):
    ...
    assert round(tdm_region_size[0] + 2 * port_len, 2) == target_cell_size, f"right hand side: {tdm_region_size[0] + 2 * port_len}, target_cell_size: {target_cell_size}"
    ...
    def perturb_and_dump(step, flip_prob=0.1, i=None):
        ...
        with DeterministicCtx(seed=42 + step + i):
            ...
            try:
                ...
                filename_h5 = dump_data_path + f"/tdm_id-{device_id}_opt_step_{step}_perturbed_{i}.h5"
                filename_yml = dump_data_path + f"/tdm_id-{device_id}_perturbed_{i}.yml"
                opt.dump_data(filename_h5=filename_h5, filename_yml=filename_yml, step=step)

                opt.plot(
                    eps_map=opt._eps_map,
                    obj=results["breakdown"]["temp1_trans"]["value"],
                    plot_filename=f"tdm_opt_step_{step}_temp1_fwd_perturbed_{i}.png",
                ...
                opt.plot(
                    eps_map=opt._eps_map,
                    obj=results["breakdown"]["temp2_trans"]["value"],
                    plot_filename=f"tdm_opt_step_{step}_temp2_fwd_perturbed_{i}.png",
                    field_key=("in_slice_1", 1.55, 1, 360),
                    field_component="Ez",
                    in_slice_name="in_slice_1",
                    exclude_slice_names=[],
                )
    ...
    for step in range(n_epoch):
        ...
        filename_h5 = dump_data_path + f"/tdm_id-{device_id}_opt_step_{step}.h5"
        filename_yml = dump_data_path + f"/tdm_id-{device_id}.yml"
        ...
        if last_design_region_dict is None:
            ...
            opt.plot(
                eps_map=opt._eps_map,
                obj=results["breakdown"]["temp1_trans"]["value"],
                plot_filename="tdm_opt_step_{}_temp1_fwd.png".format(step),
                field_key=("in_slice_1", 1.55, 1, 300),
                field_component="Ez",
                in_slice_name="in_slice_1",
                exclude_slice_names=[],
            ...
            opt.plot(
                eps_map=opt._eps_map,
                obj=results["breakdown"]["temp2_trans"]["value"],
                plot_filename="tdm_opt_step_{}_temp2_fwd.png".format(step),
                field_key=("in_slice_1", 1.55, 1, 360),
                field_component="Ez",
                in_slice_name="in_slice_1",
                exclude_slice_names=[],
...
def main():
    ...
    tdm_opt(random_seed, device, each_step, include_perturb)
