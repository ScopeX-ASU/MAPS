...
def crossing_opt(device_id, operation_device, each_step=False, include_perturb=False, perturb_probs=[0.1, 0.3, 0.5]):
    ...
    assert round(crossing_region_size[0] + 2 * port_len, 2) == target_cell_size, f"right hand side: {crossing_region_size[0] + 2 * port_len}, target_cell_size: {target_cell_size}"
    ...
    def perturb_and_dump(step, flip_prob=0.1, i=None):
        ...
        with DeterministicCtx(seed=42 + step + i):
            ...
            try:
                ...
                filename_h5 = dump_data_path + f"/crossing_id-{device_id}_opt_step_{step}_perturbed_{i}.h5"
                filename_yml = dump_data_path + f"/crossing_id-{device_id}_perturbed_{i}.yml"
                ...
                opt.plot(
                    ...
                    plot_filename=f"crossing_opt_step_{step}_fwd_perturbed_{i}.png",
    ...
    for step in range(n_epoch):
        ...
        filename_h5 = dump_data_path + f"/crossing_id-{device_id}_opt_step_{step}.h5"
        filename_yml = dump_data_path + f"/crossing_id-{device_id}.yml"
        ...
        if last_design_region_dict is None:
            ...
            opt.plot(
                eps_map=opt._eps_map,
                obj=results["breakdown"]["fwd_trans"]["value"],
                plot_filename="crossing_opt_step_{}_fwd.png".format(step),
                field_key=("in_slice_1", 1.55, 1, 300),
                field_component="Ez",
                in_slice_name="in_slice_1",
                exclude_slice_names=[],
...
def main():
    ...
    crossing_opt(random_seed, device, each_step, include_perturb)
