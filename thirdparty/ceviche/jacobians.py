...
from autograd.core import make_jvp, make_vjp
from autograd.extend import vspace
from autograd.wrap_util import unary_to_nary

from .utils import float_2_array, get_shape, get_value, get_value_arr
...
def jacobian(fun, argnum=0, mode="reverse", step_size=1e-6):
    ...
    if mode == "reverse":
        ...
        raise ValueError(
            "'mode' kwarg must be either 'reverse' or 'forward' or 'numerical', given {}".format(
                mode
            )
        )
...
def jacobian_forward(fun, x):
    ...
    ans = npa.zeros(
        (list(vals)[0].size,)
    )  # fake answer so that dont have to compute it twice
    m, n = _jac_shape(x, ans)
    if _iscomplex(x):
...
def jacobian_numerical(fn, x, step_size=1e-7):
    ...
    for i in range(m):
        ...
        jacobian[:, i] = get_value_arr(
            get_value(grad_i)
        )  # need to convert both the grad_i array and its contents to actual data.
