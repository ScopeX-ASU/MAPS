...
def port_tf_to_torch(tf_model, torch_model):
    ...
    # Conv1
    W = td["conv2d_8/kernel:0"]   # (3,3,1,1)
    b = td["conv2d_8/bias:0"]     # (1,)
    ...
    # BN1
    torch_model.bn1.weight.data.copy_(torch.from_numpy(td["batch_normalization_8/gamma:0"]))
    torch_model.bn1.bias.data.copy_(torch.from_numpy(td["batch_normalization_8/beta:0"]))
    torch_model.bn1.running_mean.data.copy_(torch.from_numpy(td["batch_normalization_8/moving_mean:0"]))
    torch_model.bn1.running_var.data.copy_(torch.from_numpy(td["batch_normalization_8/moving_variance:0"]))

    # Conv2
    W = td["conv2d_9/kernel:0"]   # (3,3,1,2)
    b = td["conv2d_9/bias:0"]     # (2,)
    ...
    # BN2
    torch_model.bn2.weight.data.copy_(torch.from_numpy(td["batch_normalization_9/gamma:0"]))
    torch_model.bn2.bias.data.copy_(torch.from_numpy(td["batch_normalization_9/beta:0"]))
    torch_model.bn2.running_mean.data.copy_(torch.from_numpy(td["batch_normalization_9/moving_mean:0"]))
    torch_model.bn2.running_var.data.copy_(torch.from_numpy(td["batch_normalization_9/moving_variance:0"]))

    # Conv3
    W = td["conv2d_10/kernel:0"]   # (3,3,2,4)
    b = td["conv2d_10/bias:0"]     # (4,)
    ...
    # BN3
    torch_model.bn3.weight.data.copy_(torch.from_numpy(td["batch_normalization_10/gamma:0"]))
    torch_model.bn3.bias.data.copy_(torch.from_numpy(td["batch_normalization_10/beta:0"]))
    torch_model.bn3.running_mean.data.copy_(torch.from_numpy(td["batch_normalization_10/moving_mean:0"]))
    torch_model.bn3.running_var.data.copy_(torch.from_numpy(td["batch_normalization_10/moving_variance:0"]))

    # Conv4
    W = td["conv2d_11/kernel:0"]   # (3,3,4,8)
    b = td["conv2d_11/bias:0"]     # (8,)
    ...
    # BN4
    torch_model.bn4.weight.data.copy_(torch.from_numpy(td["batch_normalization_11/gamma:0"]))
    torch_model.bn4.bias.data.copy_(torch.from_numpy(td["batch_normalization_11/beta:0"]))
    torch_model.bn4.running_mean.data.copy_(torch.from_numpy(td["batch_normalization_11/moving_mean:0"]))
    torch_model.bn4.running_var.data.copy_(torch.from_numpy(td["batch_normalization_11/moving_variance:0"]))

    # Dense
    W = td["dense_2/kernel:0"]     # (512, 16384)
    b = td["dense_2/bias:0"]       # (16384,)
...
path = "./thirdparty/PreFab/models/p_ANT_NanoSOI_v0.1_2.pb"
