...
if __name__ == "__main__":
    ...
    for i in range(0, num_perturb):
        perturb_widths_dict[i] = [
            round(random.uniform(0, 0.295), 2)
            for _ in range(2 * num_perturb_piller_single_side)
        ]
    ...
    for num_pillar_single_side in range(0, max_num_pillar_single_side + 1):
        ...
        device = MetaLens(
            ...
            sim_cfg=sim_cfg,
            aperture=0.3
            * ((num_pillar_single_side + num_perturb_piller_single_side) * 2 + 1),
            port_len=(1, 1),
            port_width=(
                0.3
                * ((num_pillar_single_side + num_perturb_piller_single_side) * 2 + 1),
        ...
        for perturb_time in range(num_perturb):
            for p in opt.parameters():
                ...
                extend_pillar_widths = (
                    perturb_widths_dict[perturb_time][:num_perturb_piller_single_side]
                    + pillar_widths
                    + perturb_widths_dict[perturb_time][
                        -num_perturb_piller_single_side:
                    ]
                )
                print("this is the pillar_widths: ", extend_pillar_widths, flush=True)
                ls_knots = [
                    get_mid_weight(0.05, round(extend_pillar_widths[j], 2))
                    for j in range(len(extend_pillar_widths))
                ]
                ls_knots = torch.tensor(ls_knots, device=operation_device)
                weight = torch.zeros(
                    (1 + 2 * len(extend_pillar_widths)), device=operation_device
                )
                weight[::2] = -0.05
                weight[1::2] = ls_knots
            ...
            center_atom_phase = opt.objective.phase_shift[
                ("in_slice_1", "nearfield_1", 0.85, "Hz1", 300)
            ]["phase"]
            center_atom_phase_mean = opt.objective.phase_shift[
                ("in_slice_1", "nearfield_1", 0.85, "Hz1", 300)
            ]["phase_mean"]
            center_atom_phase_dict[num_pillar_single_side].append(center_atom_phase)
            center_atom_phase_mean_dict[num_pillar_single_side].append(
                center_atom_phase_mean.item()
            )
