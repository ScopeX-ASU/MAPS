...
if __name__ == "__main__":
    ...
    for num_pillar_single_side in range(0, max_num_pillar_single_side+1):
        ...
        for perturb_time in range(num_perturb):
            perturb_widths = [round(random.uniform(0, 0.295), 2) for _ in range(num_perturb_piller_single_side)]
            pillar_widths = pillar_widths[:num_pillar_single_side + 1] + perturb_widths
            print("this is the pillar_widths: ", pillar_widths, flush=True)
            for p in opt.parameters():
                # init_weight = [-0.05, get_mid_weight(0.05, round(i.item(), 2)), -0.05]
                extend_pillar_widths = pillar_widths[::-1][:-1] + pillar_widths
            ...
            center_atom_phase = opt.objective.phase_shift[('in_slice_1', 'nearfield_1', 0.85, 1, 300)]["phase"]
            N = len(center_atom_phase)
    ...
    for key, value in center_atom_phase_dict.items():
        ...
        plt.close()
        # calculate the std along dim 1;
        std = torch.std(value, dim=1)
        mean_std = torch.mean(std)
        std_mean_list.append(mean_std.item())

    num_pillars_list = np.array(num_pillars_list)
    std_mean_list = np.array(std_mean_list)
    # save them to csv
    with open('./unitest/metaatom_phase.csv', mode='w') as file:
        writer = csv.writer(file)
        writer.writerow(["num_pillars", "std_mean"])
        for i in range(len(num_pillars_list)):
            writer.writerow([num_pillars_list[i], std_mean_list[i]])
