...
sx = (
    2 * (pad_x + dpml) + 2 + 1
)  # 2 is in the height of the sub, 1 is the max height of the ridge
...
sub = mp.Block(
    center=mp.Vector3(-sx / 2 + height_sub / 2, 0),
...
for i in range(len(widths)):
    geometry.append(
        mp.Block(
            center=mp.Vector3(-0.5 * sx + height_sub + 0.5 * height_ridge, period * i),
...
incident_power = numpy.array(
    mp.get_fluxes(flux_box)
).sum()  # this should be a list nf elements
...
refl_fr = mp.FluxRegion(
    center=mp.Vector3(-0.5 * sx + height_sub / 2, 0, 0), size=mp.Vector3(0, aperture, 0)
)
refl = sim.add_flux(fcen, fwidth, nfreq, refl_fr)

tran_fr = mp.FluxRegion(
    center=mp.Vector3(-0.5 * sx + height_sub + height_ridge + 0.5, 0),
    size=mp.Vector3(0, aperture, 0),
)
tran = sim.add_flux(fcen, fwidth, nfreq, tran_fr)

top = mp.FluxRegion(
    center=mp.Vector3(-0.5 * sx + height_sub + height_ridge + 0.25, aperture / 2),
...
far_flux_box = (
    numpy.array(
        nearfield_box.flux(
            mp.Y,
            mp.Volume(
                center=mp.Vector3(f_min / 2, f_min), size=mp.Vector3(f_min, 0, 0)
            ),
            res_ff,
        )
    )
    ...
    + numpy.array(
        nearfield_box.flux(
            mp.X,
            mp.Volume(center=mp.Vector3(f_min), size=mp.Vector3(y=2 * f_min)),
            res_ff,
        )
    )
)
...
for i in range(int(f_min // sample_step)):
    ff = sim.get_farfield(nearfield_box, mp.Vector3(i * sample_step, f_min))
...
for k in range(int(f_min // sample_step) - 1):
    ff1 = sim.get_farfield(nearfield_box, mp.Vector3(f_min, k * sample_step))
    E_field.append(numpy.array(ff1).reshape(nfreq, 6)[:, :3])
    H_field.append(numpy.array(ff1).reshape(nfreq, 6)[:, 3:])
    if k != 0:
        ff2 = sim.get_farfield(nearfield_box, mp.Vector3(f_min, -k * sample_step))
