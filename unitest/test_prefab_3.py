...
class KerasPortNet(nn.Module):
    ...
    def __init__(self):
        super().__init__()
        bn = dict(eps=1e-3, momentum=0.01, affine=True, track_running_stats=True)  # Keras 0.99 -> torch 0.01
    ...
    def forward(self, x):  # NCHW
        x = self.conv1(x); x = F.relu(x, inplace=False); x = self.pool1(x); x = self.bn1(x)
        x = self.conv2(x); x = F.relu(x, inplace=False); x = self.pool2(x); x = self.bn2(x)
        x = self.conv3(x); x = F.relu(x, inplace=False); x = self.pool3(x); x = self.bn3(x)
        x = self.conv4(x); x = F.relu(x, inplace=False); x = self.pool4(x); x = self.bn4(x)
...
def port_tf_to_torch(tf_model, torch_model):
    ...
    # Conv1
    W = td["conv2d_12/kernel:0"]   # (3,3,1,1)
    b = td["conv2d_12/bias:0"]     # (1,)
    ...
    # BN1
    torch_model.bn1.weight.data.copy_(torch.from_numpy(td["batch_normalization_12/gamma:0"]))
    torch_model.bn1.bias.data.copy_(torch.from_numpy(td["batch_normalization_12/beta:0"]))
    torch_model.bn1.running_mean.data.copy_(torch.from_numpy(td["batch_normalization_12/moving_mean:0"]))
    torch_model.bn1.running_var.data.copy_(torch.from_numpy(td["batch_normalization_12/moving_variance:0"]))

    # Conv2
    W = td["conv2d_13/kernel:0"]   # (3,3,1,2)
    b = td["conv2d_13/bias:0"]     # (2,)
    ...
    # BN2
    torch_model.bn2.weight.data.copy_(torch.from_numpy(td["batch_normalization_13/gamma:0"]))
    torch_model.bn2.bias.data.copy_(torch.from_numpy(td["batch_normalization_13/beta:0"]))
    torch_model.bn2.running_mean.data.copy_(torch.from_numpy(td["batch_normalization_13/moving_mean:0"]))
    torch_model.bn2.running_var.data.copy_(torch.from_numpy(td["batch_normalization_13/moving_variance:0"]))

    # Conv3
    W = td["conv2d_14/kernel:0"]   # (3,3,2,4)
    b = td["conv2d_14/bias:0"]     # (4,)
    ...
    # BN3
    torch_model.bn3.weight.data.copy_(torch.from_numpy(td["batch_normalization_14/gamma:0"]))
    torch_model.bn3.bias.data.copy_(torch.from_numpy(td["batch_normalization_14/beta:0"]))
    torch_model.bn3.running_mean.data.copy_(torch.from_numpy(td["batch_normalization_14/moving_mean:0"]))
    torch_model.bn3.running_var.data.copy_(torch.from_numpy(td["batch_normalization_14/moving_variance:0"]))

    # Conv4
    W = td["conv2d_15/kernel:0"]   # (3,3,4,8)
    b = td["conv2d_15/bias:0"]     # (8,)
    ...
    # BN4
    torch_model.bn4.weight.data.copy_(torch.from_numpy(td["batch_normalization_15/gamma:0"]))
    torch_model.bn4.bias.data.copy_(torch.from_numpy(td["batch_normalization_15/beta:0"]))
    torch_model.bn4.running_mean.data.copy_(torch.from_numpy(td["batch_normalization_15/moving_mean:0"]))
    torch_model.bn4.running_var.data.copy_(torch.from_numpy(td["batch_normalization_15/moving_variance:0"]))

    # Dense
    W = td["dense_3/kernel:0"]     # (512, 16384)
    b = td["dense_3/bias:0"]       # (16384,)
...
import visualkeras
path = "./thirdparty/PreFab/models/p_ANT_NanoSOI_v0.1_3.pb"
