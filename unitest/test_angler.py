...
print(
    "The simulation has {} grids per free space wavelength".format(
        int(lambda0 / dl / simulation.L0)
    )
)
...
print(
    loss(
        torch.from_numpy(np.stack([Ex, Ey, Hz], axis=0)),
        torch.from_numpy(eps_r).to(torch.cfloat),
    )
)
ex, ey = loss.Hy_to_E(
    torch.from_numpy(Hz).to(torch.cfloat).unsqueeze(0).unsqueeze(0),
    epsilon_r=torch.from_numpy(eps_r).to(torch.cfloat).unsqueeze(0).unsqueeze(0),
)
