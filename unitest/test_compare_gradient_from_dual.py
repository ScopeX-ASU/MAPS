        ...
        while local_step < num_batches:
            ...
            with amp.autocast("cuda", enabled=False):
                ...
                if local_step == 1:
                    plt.figure()
                    plt.imshow(
                        np.rot90(fwd_field[0].real.detach().cpu().numpy()), cmap="RdBu"
                    )
                    plt.colorbar()
                    plt.title("fwd_field")
                    plt.savefig(
                        f"./figs/fwd_field_{configs.model_fwd.type}.png", dpi=300
                    )
                    plt.close()

                    plt.figure()
                    plt.imshow(
                        np.rot90(adj_field[0].real.detach().cpu().numpy()), cmap="RdBu"
                    )
                    plt.colorbar()
                    plt.title("adj_field")
                    plt.savefig(
                        f"./figs/adj_field_{configs.model_fwd.type}.png", dpi=300
                    )
                    plt.close()
            ...
            for i in range(gradients.shape[0]):
                ...
                if local_step == 1 and i == 0:
                    ...
                    plt.title("gradients")
                    plt.savefig(
                        f"./figs/gradients_{configs.model_fwd.type}.png", dpi=300
                    )
                    plt.close()
