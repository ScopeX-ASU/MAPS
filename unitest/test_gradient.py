...
def test_gradient(filepath):
    ...
    with h5py.File(filepath, "r") as f:
        ...
        forward_field = torch.from_numpy(
            f["field_solutions-wl-1.55-port-in_port_1-mode-1-temp-300"][:]
        ).to("cuda")
        adjoint_field = torch.from_numpy(
            f["fields_adj-wl-1.55-port-in_port_1-mode-1"][:]
        ).to("cuda")
        # read the gradient
